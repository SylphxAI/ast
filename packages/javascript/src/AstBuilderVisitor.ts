...
import type { Position } from "@sylphlab/ast-core"; // Import base types
...
import type { ECMAScriptVisitor } from "./generated/grammar/ECMAScriptVisitor";
 ...
 */
function createPosition(ctx: {
  start: { startIndex: number };
  stop?: { stopIndex: number } | null;
}): Position {
...
export class AstBuilderVisitor
  extends AbstractParseTreeVisitor<JsNode | null>
  implements ECMAScriptVisitor<JsNode | null>
{
  // Entry point
  visitProgram(ctx: ProgramContext): Program | null {
    ...
    const body: Statement[] = sourceElements
      ? sourceElements
          .sourceElement()
          .map((el) => this.visit(el) as Statement)
          .filter((s) => s !== null)
      : [];
  ...
  visitStatement(ctx: StatementContext): Statement | null {
    ...
    const variableStatement = ctx.variableStatement();
    if (variableStatement) {
      return this.visitVariableStatement(variableStatement);
    }
  ...
  visitIfStatement(ctx: IfStatementContext): IfStatement | null {
    ...
    const alternateCtx = ctx.statement(1);
    const alternateStmt = alternateCtx ? this.visitStatement(alternateCtx) : null; // Second (optional) is alternate
  ...
  visitVariableDeclaration(ctx: VariableDeclarationContext): VariableDeclarator | null {
    ...
    const identifier: AstIdentifier = {
      ...
      text: idNode.text,
      position: {
        // Position from the terminal node itself
  ...
  visitExpressionStatement(ctx: ExpressionStatementContext): ExpressionStatement | null {
    ...
    const expressionSequence = ctx.expressionSequence();
    const expression = expressionSequence ? this.visitExpressionSequence(expressionSequence) : null;
  ...
  visitLiteral(ctx: LiteralContext): AstLiteral | null {
    ...
    const regularExpressionLiteral = ctx.RegularExpressionLiteral();

    if (ctx.NullLiteral()) {
      value = null;
    } else if (booleanLiteral) {
      value = booleanLiteral.text === "true";
      ...
      try {
        ...
        if (Number.isNaN(value)) {
          // Handle cases like '0x' which parseFloat might return NaN for
    ...
    } else if (regularExpressionLiteral) {
